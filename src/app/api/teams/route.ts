...
import { getTeams } from "@/services/sportsData/sportsDataService";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await getTeams(competitionId, refresh);
    return NextResponse.json(result);
  } catch {
    return apiError(500, "internal_error", "Não foi possível carregar os times agora. Tente novamente.");
  }
}
